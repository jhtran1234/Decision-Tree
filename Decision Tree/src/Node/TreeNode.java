...
public class TreeNode {
	...
	public TreeNode(final String attribute, final DataTable t) {
	...
	public HashSet<String> findOptions() {
		final HashSet<String> options = new HashSet<>();
	...
	/**
	 * Recursive method to constantly create new children until base Final
	 * nodes can be placed
	 */
	public void makeChildren() {
		final HashSet<String> options = findOptions();

		// for each option of the attribute, find next best entropy child and build
		for (String option : options) {
			final DataTable t = table.singleOption(attribute, option);
			...
			if (t.table.size() <= 0) {
				...
				final TreeNode node = 
						new TreeNode(t.chooseAttribute().get(0), t);
				...
				final TreeNode s = (trueCount >= falseCount) ?
						children.put(option, new Final(true, trueCount /
								(trueCount + falseCount))) :
									children.put(option, 
											new Final(false, falseCount / 
													(trueCount + falseCount)));
			}
		}
	}
